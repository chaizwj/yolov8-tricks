...
from ultralytics.nn.modules import (AIFI, C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    ...
                                    RTDETRDecoder, Segment, CBAM, SPPFCSPC, BiLevelRoutingAttention, EMA,
                                    ParNetAttention, C2f_Attention, SPPF_SEAttention, CoordAtt, C2f_ScConv, C2f_DCN, GAM_Attention, LSKblockAttention,SimAM
                                    )

#   如果 新模块比较复杂点的， 是直接 对应一个 py文件的话，就需要 这样 直接定位导入
...
from ultralytics.nn.modules.rep_block import C2f_DBB


from ultralytics.nn.head.LADetect import LADetect
from ultralytics.nn.head.SC_C_Detect import SC_C_Detect
from ultralytics.nn.head.AFPNDetect import Detect_AFPN
from ultralytics.nn.head.Detect_Efficient import Detect_Efficient
from ultralytics.nn.head.dyhead import Detect_dyhead3
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Segment,LADetect,SC_C_Detect,Detect_AFPN,Detect_Efficient,Detect_dyhead3)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Segment, Pose,LADetect,SC_C_Detect,Detect_AFPN,Detect_Efficient,Detect_dyhead3)):
            s = 256  # 2x min stride
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Segment,LADetect,SC_C_Detect,Detect_AFPN,Detect_Efficient,Detect_dyhead3):
            m.inplace = inplace
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        ##########  begin 一般 和 C2f 融合的 c2f_XXX 模块都在这里添加 ##############
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3,C2f_Attention,SPPF_SEAttention,C2f_ScConv,C2f_DCN,C2f_GhostBottleneck,C2f_GhostBottleneckV2,BasicStage, PatchEmbed_FasterNet, PatchMerging_FasterNet,C2f_DBB):
          ...
          #   内部的 有 Bottleneck模块 要有 重复次数的 ，这里都要继续添加一下
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3,C2f_Attention,C2f_ScConv,C2f_DCN,C2f_GhostBottleneck,C2f_GhostBottleneckV2,C2f_DBB):
                args.insert(2, n)  # number of repeats
                n = 1

            elif m in[BasicStage]:
                args.pop(1)
            ##########  end 一般 和 C2f 融合的 c2f_XXX 模块都在这里添加 ##############
        ...
        ################   检测头 添加 的 其中一个位置  ##############
        elif m in (Detect, Segment, Pose,LADetect,SC_C_Detect,Detect_AFPN,Detect_Efficient,Detect_dyhead3):
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Detect,LADetect,SC_C_Detect,Detect_AFPN,Detect_Efficient,Detect_dyhead3)):
                return 'detect'
