...
#   主要是在 这个 BboxLoss类中 的 forward 函数里面 的 调用  bbox_iou（）处的 参数 进行 修改
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        # forward 函数里面 的 调用  bbox_iou（）处的 参数 进行 修改,想选择哪个 变种 Iou函数 就选择哪个，
        # iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False,EIoU=True,Focal=True)
        #
        #  #  下面的 这2 行 代码 也是需要 修改 一下的
        # # 如果  采用变种 Iou ，就执行  这行代码
        # if type(iou) is tuple:
        #     loss_iou = ((1.0 - iou[0]) * iou[1].detach() * weight).sum() / target_scores_sum
        #
        # # 否则 就采用 Yolov8 默认 CIoU  ，就执行  这行代码
        # else:
        #     loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum



        ### 加入 Wise IOU后的 需要 调用bbox_iou 方法 以及 下面2行代码 的写法
        # # WIoU
        ...
        # # Inner_iou 损失函数
        # iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, inner_EIoU=True,Focal=True)
        # if type(iou) is tuple:
        #     if len(iou) == 2:
